  ...
  ({ className, onChange, inputMode, autoCapitalize, ...props }, ref) => {
    ...
    return (
      <textarea
        ...
        inputMode={inputMode}
        className={cn(
          "flex min-h-24 w-full resize-y rounded-xl border border-border/80 bg-card px-3.5 py-3 text-base shadow-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-muted-foreground/55 placeholder:font-sans placeholder:font-normal placeholder:normal-case placeholder:tracking-normal hover:border-accent/50 focus-visible:border-accent focus-visible:bg-card focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/10 aria-invalid:border-destructive aria-invalid:ring-destructive/10 disabled:cursor-not-allowed disabled:bg-muted/45 disabled:opacity-60 md:text-sm",
          className,
        )}
        ref={ref}
        {...props}

...
export const Route = createFileRoute("/admin/department-manager")({
  component: DepartmentManagerPage,
  head: () => ({
    meta: [
      { title: "Department Manager | Radiant Control Center" },
      { name: "description", content: "Create and manage company departments used across employee records and designations." },
      { property: "og:title", content: "Department Manager | Radiant Control Center" },
      { property: "og:description", content: "Create and manage company departments used across employee records and designations." },
...
const MODULE = "Department Manager";
...
function useDepartments() {
  ...
  type Payload = Omit<Department, "id">;
  ...
  const addMut = useMutation({
    mutationFn: async (p: Payload) => {
      ...
      const { error } = await supabase.from("departments" as never).insert(toRow(p) as never);
      if (error) throw error;
  ...
  const updateMut = useMutation({
    mutationFn: async ({ id, p }: { id: string; p: Payload }) => {
      const { error } = await supabase.from("departments" as never).update(toRow(p) as never).eq("id", id);
      if (error) throw error;
  ...
  const toggleMut = useMutation({
    mutationFn: async ({ id, enabled }: { id: string; enabled: boolean }) => {
      const { error } = await supabase.from("departments" as never).update({ enabled } as never).eq("id", id);
      if (error) throw error;
...
function DepartmentManagerPage() {
  const { items, addMut, updateMut, toggleMut, deleteMut } = useDepartments();
          ...
          <Button
            ...
            onClick={() =>
              downloadCsv(
                ...
                [{ key: "name", header: "Department" }, { key: "enabled", header: "Enabled" }],
              )
            }
        ...
        <div className="overflow-x-clip">
          <table className="ios-table w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-5 py-12 text-center text-sm text-muted-foreground">No departments found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <DepartmentFormDialog
        ...
        onSubmit={async (p) => {
          try {
            ...
            return e instanceof Error ? e.message : "Could not add department";
          }
        }}
      />
      ...
      <DepartmentFormDialog
        ...
        onSubmit={async (p) => {
          ...
          try {
            ...
            return e instanceof Error ? e.message : "Could not update department";
          }
        }}
      />
...
function DepartmentFormDialog({
  ...
  onSubmit: (p: Omit<Department, "id">) => Promise<string | null>;
}) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Departments group designations and employees across the org.</DialogDescription>
        </DialogHeader>

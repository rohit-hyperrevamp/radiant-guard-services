 ...
 *
 * - Real OTPs use the same MSG91 Widget workflow as BBDO — 4 digit codes.
...
export const verifyLoginOtp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: true }> => {
    ...
    if (!data.accessToken) throw new Error("Missing OTP verification token. Request a new code.");

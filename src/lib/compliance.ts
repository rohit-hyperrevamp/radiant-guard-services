export async function fetchComplianceIssues(
  ...
  const [
    ...
    (uIds ? supabase.from("units").select("id, code, name, status, customer_id, branch_id, gst_number, latitude, longitude, contract_end_date, emergency_contact_mobile, uniform_included, uniform_fee_amount").in("id", uIds) : supabase.from("units").select("id, code, name, status, customer_id, branch_id, gst_number, latitude, longitude, contract_end_date, emergency_contact_mobile, uniform_included, uniform_fee_amount")),
    (cIds ? supabase.from("customers").select("id, code, name, status, billing_email, billing_phone, contract_end_date").in("id", cIds) : supabase.from("customers").select("id, code, name, status, billing_email, billing_phone, contract_end_date")),
    (uIds ? supabase.from("client_contracts").select("id, contract_code, unit_id, status, approval_status, record_type, start_date, end_date, expiry_date").in("unit_id", uIds) : supabase.from("client_contracts").select("id, contract_code, unit_id, status, approval_status, record_type, start_date, end_date, expiry_date")),
    (uIds ? supabase.from("candidates").select("id, full_name, employee_code, status, is_enabled, is_disabled, role_key, non_billable, unit_id, date_of_birth, aadhaar_number, pan_number, bank_account_number, email, offboarded_at, esic_card_url").in("unit_id", uIds) : supabase.from("candidates").select("id, full_name, employee_code, status, is_enabled, is_disabled, role_key, non_billable, unit_id, date_of_birth, aadhaar_number, pan_number, bank_account_number, email, offboarded_at, esic_card_url")),
    (uIds ? supabase.from("attendance_entries").select("candidate_id, unit_id, code").eq("entry_date", todayIso).in("unit_id", uIds) : supabase.from("attendance_entries").select("candidate_id, unit_id, code").eq("entry_date", todayIso)),
    supabase.from("attendance_codes").select("code, label, counts_as_present, enabled"),
    (uIds ? supabase.from("attendance_sheets").select("id, unit_id, status, period_start, period_end").gte("period_start", monthStart).in("unit_id", uIds) : supabase.from("attendance_sheets").select("id, unit_id, status, period_start, period_end").gte("period_start", monthStart)),
    (uIds ? supabase.from("payroll_runs").select("id, unit_id, status, payroll_status, invoice_status, period_start, period_end").gte("period_start", monthStart).in("unit_id", uIds) : supabase.from("payroll_runs").select("id, unit_id, status, payroll_status, invoice_status, period_start, period_end").gte("period_start", monthStart)),

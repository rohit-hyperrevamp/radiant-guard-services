...
ALTER TABLE public.candidate_units
  ADD COLUMN IF NOT EXISTS designation_assigned_at timestamptz;

UPDATE public.candidate_units
SET designation_assigned_at = created_at
WHERE designation_id IS NOT NULL
  AND designation_assigned_at IS NULL;

CREATE OR REPLACE FUNCTION public.set_candidate_unit_designation_assigned_at()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.designation_id IS DISTINCT FROM OLD.designation_id THEN
    NEW.designation_assigned_at := CASE WHEN NEW.designation_id IS NULL THEN NULL ELSE now() END;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_candidate_unit_designation_assigned_at ON public.candidate_units;
CREATE TRIGGER trg_candidate_unit_designation_assigned_at
BEFORE UPDATE OF designation_id ON public.candidate_units
FOR EACH ROW EXECUTE FUNCTION public.set_candidate_unit_designation_assigned_at();
...
AS $$
  ...
  current_contract AS (
    SELECT DISTINCT ON (cc.unit_id) cc.unit_id, cc.id AS contract_id
    FROM public.client_contracts cc
    WHERE cc.record_type = 'client'
      AND cc.status = 'active'
      AND cc.start_date <= CURRENT_DATE
  ...
  SELECT c.id, c.full_name, NULLIF(c.employee_code, ''), NULLIF(c.candidate_code, ''),
         cu.unit_id, u.name, u.code, COALESCE(cust.name, 'Organization pending'),
         cu.designation_id, d.name, cc.contract_id,
         COALESCE(cu.designation_assigned_at, cu.created_at)::date
  FROM public.candidate_units cu
  JOIN public.candidates c ON c.id = cu.candidate_id
  JOIN public.units u ON u.id = cu.unit_id AND u.is_billable IS DISTINCT FROM false
  LEFT JOIN public.customers cust ON cust.id = u.customer_id
  ...
  WHERE c.is_enabled = true
    ...
    AND cu.designation_id IS NOT NULL
    AND NOT EXISTS (
      SELECT 1 FROM public.contract_resources cr
      WHERE cr.contract_id = cc.contract_id AND cr.designation_id = cu.designation_id
    )
    AND (
      public.is_admin_user()
      OR v.role_key IN ('finance', 'accounts')

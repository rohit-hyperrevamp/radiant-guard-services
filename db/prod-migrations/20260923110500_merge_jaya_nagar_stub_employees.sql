...
-- A guard may hold only one regular posting, so Jaya Nagar becomes the regular posting
-- and the previous site stays as an additional posting.

BEGIN;

-- 1) Drop the stub postings so the unit slots are free
...
-- 2) Jaya Nagar becomes the regular posting for both real employees
...
-- 3) Move the Jaya Nagar muster onto the real employees

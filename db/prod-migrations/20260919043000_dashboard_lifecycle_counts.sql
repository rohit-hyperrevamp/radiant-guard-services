-- Collapse the dashboard's attendance/payroll/invoice lifecycle into one query.
-- This preserves the charter unit scope but avoids downloading thousands of
-- unit/employee rows and issuing large client-side IN queries before painting.
create or replace function public.dashboard_lifecycle_counts(p_year integer, p_month integer)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  periods as (
    select
      su.unit_id,
      case
        when coalesce(su.window_start_day, 1) <= 1
          or coalesce(su.window_end_day, 0) <= 0
          or su.window_end_day >= su.window_start_day
...
revoke all on function public.dashboard_lifecycle_counts(integer, integer) from public;
grant execute on function public.dashboard_lifecycle_counts(integer, integer) to authenticated;
grant execute on function public.dashboard_lifecycle_counts(integer, integer) to service_role;

-- Invoice numbers are issued per state, so a final invoice may never cover
-- units billed in different states. Enforce it in the allocator itself.
...
begin
  if not current_user_has_permission('invoicing', null, 'edit') then
